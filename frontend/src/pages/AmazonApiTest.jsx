...
const AmazonApiTest = () => {
  ...
  const handleTest = async () => {
    ...
    const payload = {
      ...
      query_params: endpoint === 'search' 
        ? { query, page: '1', country: 'US', sort_by: 'RELEVANCE' }
        : { asin: query, country: 'US' }
    };

    try {
      const response = await api.post('/amazon/test', payload);
  ...
  return (
    <div className="h-full flex flex-col bg-gray-950">
      <div className="px-6 py-4 border-b border-gray-800 bg-gray-900 flex-shrink-0">
        <h1 className="text-base font-semibold text-white tracking-tight">Amazon API Playground</h1>
        <p className="text-xs text-gray-400 mt-0.5">Test real-time Amazon data scraping endpoints</p>
      </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-5 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <div>
              ...
              >
                <option value="search">Product Search (GET /search)</option>
                <option value="details">Product Details (GET /product-details)</option>
              </select>
            </div>
            
            <div className="md:col-span-2">
              <label className="block text-xs font-medium text-gray-400 mb-1">
                {endpoint === 'search' ? 'Search Query (e.g. Dell XPS)' : 'Amazon ASIN (e.g. B07ZPKBL9V)'}
              </label>
              <div className="flex gap-2">
                <input 
                  ...
                  placeholder={endpoint === 'search' ? "Enter product name..." : "Enter ASIN..."}
                ...
                {result.data?.products && result.data.products.length > 0 ? (
                  <table className="w-full text-left border-collapse min-w-[800px]">
                    <thead className="bg-gray-950 sticky top-0 z-10 border-b border-gray-800">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-xs font-semibold text-gray-400">Sales Volume</th>
                        <th className="px-4 py-3 text-xs font-semibold text-gray-400">Availability</th>
                      </tr>
                    </thead>
                        ...
                        <tr key={i} className="hover:bg-gray-800/30 transition-colors">
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              {p.product_photo && <img src={p.product_photo} alt="Product" className="w-10 h-10 object-contain bg-white rounded p-1" />}
                              <div>
                                <p className="text-sm font-medium text-gray-200 line-clamp-2 max-w-sm" title={p.product_title}>{p.product_title}</p>
                                <a href={p.product_url} target="_blank" rel="noreferrer" className="text-xs text-blue-400 hover:underline">View on Amazon</a>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-3 text-sm font-semibold text-green-400">{p.product_price || 'N/A'}</td>
                          <td className="px-4 py-3 text-sm text-yellow-400 font-medium">⭐ {p.product_star_rating || 'N/A'}</td>
                          <td className="px-4 py-3 text-sm text-gray-300">{p.product_num_ratings?.toLocaleString() || '0'}</td>
                          <td className="px-4 py-3 text-sm text-gray-300">{p.sales_volume || '-'}</td>
                          <td className="px-4 py-3 text-xs text-gray-400">{p.product_availability || p.delivery || 'In Stock'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  <div className="p-6">
                    <div className="flex items-start gap-6">
                      <img src={result.data.product_photo} alt="Product" className="w-32 h-32 object-contain bg-white rounded p-2" />
                      <div>
                        <h2 className="text-lg font-semibold text-white">{result.data.product_title}</h2>
                        <div className="mt-2 flex items-center gap-4">
                          <span className="text-xl font-bold text-green-400">{result.data.product_price}</span>
                          <span className="text-sm font-medium text-yellow-400">⭐ {result.data.product_star_rating} ({result.data.product_num_ratings} reviews)</span>
                        </div>
                        <p className="mt-4 text-sm text-gray-400 max-w-2xl">{result.data.product_description || 'No description available.'}</p>
                      </div>
                    </div>
                  </div>
                ) : (
