...
def main():
    ...
    # Update existing records to EXISTING_VERIFIED
    ...
    for product in products:
        ...
        if not amz_exists:
            price = generate_demo_price(product.product_id, base_price, True)
            db.add(CompetitorPriceHistory(
                product_id=product.product_id,
                competitor_name="Amazon",
                competitor_product_name=product.product_name,
        ...
        if not fk_exists:
            price = generate_demo_price(product.product_id, base_price, False)
            db.add(CompetitorPriceHistory(
                product_id=product.product_id,
                competitor_name="Flipkart",
                competitor_product_name=product.product_name,
